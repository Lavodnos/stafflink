# UI StaffLink migration status (frontend)

- Adopted StaffLink look & feel (light/dark), tokens moved to `src/styles/tailwind.css` with navy dark palette (#0b1220/#0f172a backgrounds, #1f2a3d borders, #e8eefc text). Inputs/cards/pills/buttons updated; Shell/Header/Sidebar use new colors.
...
- Lists reworked to StaffLink tables:
...
- Dashboard: added StaffLink-style metrics + charts using react-apexcharts. Metrics from react-query data (campaigns, links, candidates, blacklist). Chart 1 placeholder trend (static). Chart 2 links status active/inactive (live counts). Dark mode aware.
...
- Dark mode adjusted to match StaffLink demo; contrast improved vs previous.
