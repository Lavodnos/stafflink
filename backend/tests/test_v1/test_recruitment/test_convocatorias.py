...
class ConvocatoriaViewSetTests(APITestCase):
    def setUp(self) -> None:
        self.campaign = create_campaign()
        self.owner_id = uuid.uuid4()
        self.other_owner_id = uuid.uuid4()
        self.main_link = create_convocatoria(
            self.campaign,
            slug="primary-link",
            owner_id=self.owner_id,
        )
        self.other_link = create_convocatoria(
            self.campaign,
            slug="secondary-link",
    ...
    def test_list_filters_by_owner_when_only_read_own(self) -> None:
        url = reverse("convocatorias-list")
        response = self.client.get(
            url,
            **self._auth_headers(
                user_id=self.owner_id, permissions=["convocatorias.read"]
            ),
        )
    ...
    def test_expire_action_allows_owner_with_expire_own(self) -> None:
        url = reverse("convocatorias-expire", args=[self.main_link.pk])
        response = self.client.post(
            url,
            **self._auth_headers(
                user_id=self.owner_id, permissions=["convocatorias.close"]
            ),
        )
    ...
    def test_non_owner_cannot_expire_without_global_permission(self) -> None:
        ...
        response = self.client.post(
            url,
            **self._auth_headers(
                user_id=self.other_owner_id, permissions=["convocatorias.close"]
            ),
        )
    ...
    def test_list_all_when_user_has_read_all_permission(self) -> None:
        ...
        response = self.client.get(
            ...
            **self._auth_headers(
                ...
                permissions=["convocatorias.read", "convocatorias.manage"],
            ),
        )
