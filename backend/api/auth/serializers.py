...
class LoginSerializer(serializers.Serializer):
    ...
    def validate_username_or_email(self, value: str) -> str:
        ...
        if not candidate or len(candidate) < 3:
            raise serializers.ValidationError("Debe ingresar un usuario o correo válido.")
        return candidate

    def validate(self, attrs):  # type: ignore[override]
        if settings.IAM_CAPTCHA_REQUIRED and not attrs.get("captcha_token"):
            raise serializers.ValidationError({"captcha_token": "Se requiere CAPTCHA para esta aplicación."})
        return attrs
